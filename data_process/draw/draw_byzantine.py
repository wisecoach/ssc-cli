...
def draw_base():
    scale = 1.25
    # 读取数据
    data = pd.read_csv('byzantine.csv')
    ...
    ax1.set_xlabel('拜占庭节点数量', fontsize=label_size*scale)
    ax1.set_ylabel('吞吐量', fontsize=label_size*scale)
    ax1.tick_params(axis='both', which='major', labelsize=tick_size*scale, direction='in')
    ax1.set_ylim(0, 120)

    ax2.plot(x_t, data['eth_latency'], marker='s', linestyle='--', label='ETH', markersize=marker_size*scale, linewidth=2)
    ax2.plot(x_t, data['hmy_latency'], marker='s', linestyle='--', label='HMY', markersize=marker_size*scale, linewidth=2)
    ax2.plot(x_t, data['ssc_latency'], marker='s', linestyle='--', label='CGM-PBFT', markersize=marker_size*scale, linewidth=2)
    ax2.plot(x_t, data['ecce_latency'], marker='s', linestyle='--', label='CGM', markersize=marker_size*scale, linewidth=2)
    ...
    # 保存图片
    plt.savefig('images/byzantine.png')
    plt.show()
